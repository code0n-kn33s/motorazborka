  ...
  async update(req, res, next) {
    try {
      ...
      if (device[0] === 1) {
        return res
          .status(200)
          .json({ message: "Обновление Детали " + id + " прошло успешно" });
      } else {
        return res
          .status(404)
          .json({ message: "Модель " + id + " не найдено" });
      }
    } catch (error) {
      next(ApiError.badReq("$ERR: -->" + error.message)); // передаем сообщение, которое лежит в ошибке
    }
  ...
  async delete(req, res, next) {
    try {
      ...
      if (type === 1) {
        return res
          .status(200)
          .json({ message: "Удаление Детали " + id + " прошло успешно" });
      } else {
        return res
          .status(404)
          .json({ message: "Модель " + id + " не найдена" });
      }
    } catch (error) {
      console.error(error);
      return res
        .status(500)
        .json({ error: "Произошла ошибка сервера при удалении записи." });
    }
  }
}
